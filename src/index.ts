...
console.time('Constructed documents')
for (let i = 0; i < 400; i++) {
  comments.insertOne({
    text: `ind ${i}`,
    by: 'me',
    parent:
      i === 0
        ? ''
        : comments.docs[randomIntFromInterval(0, comments.docs.length - 1)].id,
  })
}
console.timeEnd('Constructed documents')

// Returns the populate results
comments.find([
  {
    key: 'by',
    operation: '===',
